...
def get_user_bills(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
...
@router.post("/pay")
def pay_bill(data: BillPay, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
