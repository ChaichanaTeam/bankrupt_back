...
@router.post("/create")
def create_saving_account(data: Saving_Account_creation, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    try:
...
def get_saving_accounts(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
...
@router.post("/topUp")
def topUp_saving_account(data: Saving_Account_TopUp, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def decrease_saving_account(data: Saving_Account_TopUp, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
